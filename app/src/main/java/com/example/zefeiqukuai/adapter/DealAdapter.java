...
public class DealAdapter extends MyRecyclerAdapter<DealBean.DataBean> {
    public DealAdapter(Context mContext, ArrayList<DealBean.DataBean> dataBeans, int content_gong) {
        super(mContext, dataBeans, content_gong);
    }

    @Override
    public void convert(RecyclerViewHolder holder, DealBean.DataBean data, int position) {
        ...
        if (status == 0)
        {
            holder.setText(R.id.content_gong1_btn,"代付款")
                    .setBackgroundResource(R.id.content_gong1_btn,R.drawable.bg_btn_shen_zhong);
        }else if (status == 1){
            holder.setText(R.id.content_gong1_btn,"待确认")
